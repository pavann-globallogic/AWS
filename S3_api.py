...
def create_s3_session():
    try:
        """Create a session using your AWS credentials 
         Returns:
        boto3.resource: A Boto3 S3 resource object if successful, None otherwise."""
...
def create_s3_bucket(s3, bucket_name, region='us-east-1'):
    """ Returns:
        dict: The response from the bucket creation request.
    """
    try:
...
def upload_file(s3, bucket_name, file_name, object_name=None):
    ...
    try:
        ...
        logger.info(f"{file_name} has been uploaded to {bucket_name}/{object_name}")
    except Exception as e:
        logger.error(f"Failed to upload {file_name} to {bucket_name}/{object_name}: {e}")
...
def download_file(s3, bucket_name, object_name, file_name=None):
    ...
    try:
        ...
        logger.info(f"{object_name} has been downloaded from {bucket_name} to {file_name}")
    except Exception as e:
...
def delete_file(s3, bucket_name, object_name):
    try:
        ...
        logger.info(f"{object_name} has been deleted from {bucket_name}")
    except Exception as e:
...
def read_s3_object(s3, bucket_name, object_key):
    ...
    try:
        ...
        logger.error(f"Error reading object {object_key}: {e}")

# Function to add tags to an S3 object
def add_tags_to_object(s3, bucket_name, object_key):

    """Returns:
        dict: The response from the tagging request.
    """
    ...
    try:
        ...
        logger.info(f"Tags added to object {object_key}")
    except Exception as e:
        logger.error(f"Error adding tags to object {object_key}: {e}")
